"""Run exactly the commands docs/install/index.md gives, in a sandbox, then check one memory works.
    ...
    python tools/run_install_page.py <workdir> [--find-links DIR] [--constraint SPEC] [--keep-uv] [--expect-fail]
...
PyPI; `--constraint` pins inspeximus for the control run (3.9.5 has no `install --all`, so it must fail).
...
def main():
    ...
    win = os.name == "nt"
    block = page_block("windows" if win else "posix")
    if win:
    ...
    launch_cwd.update(cursor=home, windsurf=home, cline=home, antigravity=home)
    if r.returncode != 0:
