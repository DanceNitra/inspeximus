...
def build(src_path: str | None = None) -> str:
    """`src_path` lets a caller point at a COPY of core.py.

    The control that proves --check can fail used to edit the tracked docs/CORE_MAP.md and restore
    it, which raced any other test reading that file under pytest-xdist and left the repository dirty
    if it crashed. A guard that has to damage the repository to prove it works is one nobody will run
    twice.
    """
    src = io.open(src_path or SRC, encoding="utf-8").read()
    tree = ast.parse(src)
    lines = src.splitlines()
    m = measure(src, tree)
...
def main() -> int:
    ...
    ap.add_argument("--src", help="read this instead of inspeximus/core.py (for a copy)")
    ap.add_argument("--out", help="compare against or write this instead of docs/CORE_MAP.md")
    ...
    if a.check:
        if not os.path.exists(out_path):
            print(f"REFUSED: {out_path} does not exist. Run without --check to create it.")
            return 1
        if io.open(out_path, encoding="utf-8").read().replace("\r\n", "\n") == fresh:
        ...
        import difflib
        on_disk = io.open(out_path, encoding="utf-8").read().replace(chr(13) + chr(10), chr(10)).splitlines()
    ...
    print(f"wrote {out_path} ({len(fresh):,} bytes)")
    return 0
