...
def _prose_number_agrees():
    ...
    So a row could pin `73` and describe it as "The MCP server exposes 98 tools", and pass -- twice,
...
NUMBER_CLAIMS = [
    ...
    _c("cc-tool-count", "claude-code.html", ["133"],
       "<h2>133 tools any MCP host can call</h2>",
       "The MCP server exposes 133 tools", "REPRODUCIBLE", "python claims_audit.py --numbers",
    ...
    _c("readme-echo-n-and-version", "README.md", ["30", "2.0.11", "2026", "2.0.18"],
       ...
       "REPRODUCIBLE", "curl -s https://pypi.org/pypi/mem0ai/json"),
    _c("readme-mcp-tool-count", "README.md", ["133"],
       "**133 tools**",
       "The MCP server exposes 133 tools", "REPRODUCIBLE", "python claims_audit.py --numbers",
       "Checked against the live @mcp.tool() count by _live_consistency(), not read from here."),
    ...
    _c("readme-adapter-conformance", "README.md", ["14", "14", "0"],
       "**14 of 14 verified against current upstream, 0 recorded broken.**",
       "Framework adapters: 14 of 14 verified against current upstream, 0 recorded broken",
    ...
    _c("readme-compliance-controls", "README.md", ["22"],
       "into 22 article-labelled",
       "The compliance overlay has twenty-two controls: the seven memory rows, the seven action-ledger rows, "
       "the seven documentation and deployer rows and the partitions row",
       "REPRODUCIBLE", "python probes/what_the_agent_knew_when_it_acted.py",
       "The probe's receipt carries compliance_controls; the README number is a copy of that field."),
    _c("readme-operator-fields", "README.md", ["24"],
       "24 provider fields marked",
       "The Annex IV skeleton marks 24 provider fields as OPERATOR INPUT REQUIRED: len(OPERATOR_FIELDS)",
    ...
    _c("readme-mcp-tools", "docs/DEEP_DIVE.md", ["133"],
       "`inspeximus-mcp`, 133 tools",
       "The MCP server exposes 133 tools", "REPRODUCIBLE",
       'python -c "import re,pathlib;print(len(re.findall(chr(64)+chr(112)+chr(99)+chr(112)+chr(46)+'
    ...
    # ---------------------------------------------------------- MCP_LISTINGS.md
    _c("mcp-tool-count", "MCP_LISTINGS.md", ["133"],
       "`inspeximus-mcp`, 133 tools",
       "The MCP server exposes 133 tools", "REPRODUCIBLE",
    ...
    _c("mcp-tool-list", "MCP_LISTINGS.md", ["133"],
       "**Tools (133):**",
    ...
    # -------------------------------------------------------------- index.html
    _c("site-mcp-tools-counter", "index.html", ["133", "0"],
       'data-count="133">0</b><span>MCP tools',
       "Homepage counter: 133 MCP tools", "REPRODUCIBLE", "python claims_audit.py --numbers",
    ...
    _c("site-mcp-tools-heading", "index.html", ["133"],
       "133 tools any MCP host can call",
       "Homepage heading: 133 MCP tools", "REPRODUCIBLE", "python claims_audit.py --numbers"),
    _c("site-coverage-matrix", "index.html", ["3.4.0", "38", "36", "2", "0"],
       "prints the matrix since release 3.4.0: 38 rows, 36 with a library call that produces the evidence, 2 not applicable, 0 not covered",
       "The coverage matrix on a fresh store at 3.4.0 and later: 38 rows, 36 in scope with a producing call, 2 not applicable, 0 not covered",
    ...
    _c("site-art15-evidence-rows", "index.html", ["3.6.0", "3"],
       ...
       "REPRODUCIBLE", "python tools/gen_robustness_evidence.py --check",
       note="The rows are read from the probe receipts by tools/gen_robustness_evidence.py, never typed; "
            "tests/test_the_two_partial_rows_close_art_15_evidence_rows_and_the_qms_register.py pins the count and the STALE control."),
    _c("site-adapters", "index.html", ["14", "0", "14", "14"],
       'data-count="14">0</b><span>integrations, 14 of 14 verified upstream',
       "Homepage counter: 14 integrations, 14 of 14 verified against current upstream", "REPRODUCIBLE",
       "python claims_audit.py --numbers",
       ...
       "pydantic_ai)."),
    _c("site-integration-conformance", "index.html", ["14", "14", "0"],
       "14 of 14 verified against current upstream, 0 recorded broken",
       "14 of 14 framework adapters verified against current upstream; 0 recorded broken",
...
NON_CLAIM_TOKENS = {
    "erasure.html": {
        ...
        # NUMBER_CLAIMS (erasure-*). Checked by `python tools/page_runs.py erasure`; a rebuild of the
        # page re-runs the flow and re-registers these per-run literals.
        ...
        "90": (2, "the leading digits of the writer's public key 90db34..., in writer-key's output and the --expected-pubkey argument"),
        "029": (2, "record id 029e96ab3b, printed at write and in the dry run"),
    ...
    "audit-trail.html": {
        ...
        "0600": (1, "the file mode writer-key prints for the secret"),
        "7": (3, "the leading digit of the writer's public key 7c4e59..., in writer-key's output, the trust_seeds line and the --expected-pubkey argument"),
        ...
        "3": (1, "the record hash 3e42f575d77f, a per-run literal"),
    ...
    "README.md": {
        ...
        "0,": (1, "'Two signers, two claims, one artifact' prose; the digit is part of a sentence, "
               "not a measurement"),
        "0.3": (1, "the authority example: source={'doc': ..., 'authority': 0.3} is an example "
                "literal a reader changes, not a measurement (2026-09-20)"),
        ...
        "14": (1, "EU AI Act ARTICLE number (human oversight) in the action-ledger section, not a quantity"),
        "22": (1, "GDPR ARTICLE number (automated decisions) in the action-ledger section, not a quantity"),
        ...
        "73": (2, "EU AI Act ARTICLE number (serious incidents): the action-ledger section and the deployer-report "
        ...
        "133,": (1, "the MCP tool count followed by a comma in prose; the claim itself is '73'. "
    ...
    "index.html": {
        # The evidence-layer section (site commit d876b69, 2026-09-21): article numbers of the AI Act and GDPR,
        # the version the layer started at, and the RFC number, none of them a quantity.
        "2.29": (1, "the release the evidence layer started at, a version"),
        "3161": (1, "RFC 3161, the timestamp protocol, a document number"),
        "12,": (1, "EU AI Act Art. 12 in 'Art. 12, 19', an article number"),
        "19": (1, "EU AI Act Art. 19, an article number"),
        "13": (2, "GDPR Art. 13 in 'Art. 13 to 22' and 'Art. 13/14', article numbers"),
        "22": (1, "GDPR Art. 22 in 'Art. 13 to 22', an article number"),
        "15,": (1, "GDPR Art. 15 in 'Art. 15, 20', an article number"),
        "20": (1, "GDPR Art. 20, an article number"),
        "16": (1, "GDPR Art. 16, an article number"),
        "21": (1, "GDPR Art. 21, an article number"),
        "28": (1, "GDPR Art. 28 in 'Art. 28 to 35', an article number"),
        "35,": (1, "GDPR Art. 35 in 'Art. 28 to 35, AI Act Art. 4', an article number"),
        "4,": (1, "EU AI Act Art. 4 in 'Art. 4, 5, 25, 43', an article number"),
        "5,": (1, "EU AI Act Art. 5, an article number"),
        "25,": (1, "EU AI Act Art. 25, an article number"),
        "43": (1, "EU AI Act Art. 43, an article number"),
        "30": (1, "GDPR Art. 30, the register, an article number"),
        "72": (1, "the 72-hour clock of GDPR Art. 33, a legal deadline"),
        "26,": (1, "EU AI Act Art. 26 in 'Art. 26, 72, 73', an article number"),
        "72,": (1, "EU AI Act Art. 72 in 'Art. 26, 72, 73', an article number"),
        "73": (2, "EU AI Act Art. 73, an article number, twice in the reports card"),
        ...
        "17": (3, "GDPR Art. 17 -- an article number, in the erasure card and in the evidence-layer rights card; "
                  "AI Act Art. 17 in the 3.6.0 sentence of the evidence section"),
        "15": (1, "AI Act Art. 15 -- an article number, in the 3.6.0 sentence of the evidence section"),
