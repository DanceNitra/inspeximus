...
def test_no_writer_is_told_a_record_landed_that_did_not(workers):
    ...
    assert landed >= claimed, (
        ...
        "So a non-zero count here is the whole explanation.\n"
        "IF THE LOCK WAS HELD, the next suspect has a name and a fix, both dated 2026-09-12. "
        "`_load_from_disk` used to read the file and stamp `_file_sig` afterwards, with no lock "
        "across the two, so a write landing during the read left the handle holding the old records "
        "under the new signature; `_save` compared them, saw no change, and rewrote the whole store "
        "from the stale view. Measured with 30 interleaved rounds of 12 writers: the old ordering "
        "lost 9 of 2,880 records, the fix lost 0 of 2,864. If this fails with the lock held on a "
        "tree that already stamps before the read, the cause is a third mechanism, not that one.\n"
        "Two readings that have been TESTED AND DID NOT HOLD, so do not spend the day on them: the "
        "guard's (mtime_ns, size) fields, which collide on roughly 8 percent of same-length writes "
        "here yet added no loss once the ordering was fixed (0 of 2,840 with st_ino added); and "
        "coarse mtime alone, which "
        "probes/a_same_size_write_inside_one_mtime_tick_is_invisible.py forces up to a 60 s tick "
        "without losing a record.\n"
        "The shape is a second, WEAKER hint: a whole worker means the losing handle never reloaded."
