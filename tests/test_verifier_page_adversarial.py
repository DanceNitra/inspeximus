...
@GROUP
@finding("F1", "the page picks the verifier from fields the file controls; a file with markers of both kinds "
               "is checked only as a certificate")
...
@GROUP
@finding("F1", "the reverse direction: `kind` routes to the bundle verifier even when the file is a failing "
               "certificate")
...
@GROUP
@finding("F2", "the verdict is not cleared when the text it was computed for is replaced")
...
@GROUP
@finding("F3", "runFile's non-UTF-8 branch shows a verdict without bumping runId, so an older run overwrites it")
...
@GROUP
@finding("F3", "Clear hides the result without bumping runId, so a run in flight shows it again")
...
@finding("F4", "the page refuses a type Python accepts and says Python would stop with an error")
...
@finding("F5", "MAX_DEPTH 200 is well under what json.loads parses")
...
@GROUP
@finding("F6", "an unsettled signature skips the 'signed by an unexpected key' comparison, which needs no crypto")
...
@GROUP
@finding("F8", "json.loads returns ONE shared NaN object, so Python's [NaN] == [NaN] is True (list equality "
               "tries identity first); pyEq says False, and the page reaches a verdict Python never does")
