...
3. **2.44.0, shipped 2026-09-18: Art. 20, 21, 86 and GDPR 33/34.** `corrective_action()` with
   ...
   covered.
4. **2.45.0, shipped as 3.3.0 on 2026-09-21: Art. 18, 43/47/48, 4, 5, 25.**
   `attest_documentation_retention()` over the five Art. 18(1) documents with the ten-year end and
   the declaration linked; `record_declaration()` with every Annex V item, the Art. 43 procedure
   and the Art. 48 marking, `declaration_document(seq)` as the machine-readable form;
   `record_literacy()` with the measure, audience and considerations, no score;
   `record_attestation()` per Art. 5(1) class with the register naming the classes with none;
   `record_responsibilities()` with the parties, roles, 25(1) trigger and 25(2) items. MCP:
   `record_literacy`, `literacy_register`, `record_attestation`, `attestation_register`,
   `record_responsibilities`, `responsibilities_register`, `record_declaration`,
   `declaration_document`, `attest_documentation_retention`. 33 covered, 3 not covered.
5. **2.46.0, shipped as 3.4.0 on 2026-09-21: GDPR 13/14, 20, 21, 28.** `record_notice()` with the
   items given and the items missing; `export_subject(basis="portability")` labelled as the Art. 20
   response with format version 1 and per-record portable flags; `object_processing()` on the store
   withholds the subject's records from recall, later writes included, resolved like erasure, with
   `resolve_objection()` (upheld, or overridden on stated grounds, never for direct marketing);
   `record_processing_role()` with the controller, the 28(3) instructions and the 28(2)
   sub-processors. MCP: `record_notice`, `notice_register`, `record_objection`, `resolve_objection`,
   `objections`, `record_processing_role`, `processing_roles`. 36 covered, 0 not covered.
