...
def route(text: str, key: str = "", object: str = "", context: str = "", policy: str = "safe",
    ...
    "trusting" always restores; any other `policy` is refused. Returns {intent, action, key, ...}
...
def admissibility_preconditions() -> dict:
    ...
    The third asks the STORE, not how it was opened: a store written without receipts and opened with
    INSPEXIMUS_RECEIPTS=1 has an empty chain because it never took the precondition, and reports
    `applicable: false` here (verify_writes still names the empty chain). A precondition that cannot apply
    reports `applicable: false` and is left out of `ok` -- a question that did not arise has not been
    answered -- and `ok` is false when none applies.

    The layer and the first two invariants are @Stratogain's (safal207/Causal-Memory-Layer#289); the
    third comes from our own 450-record decision store, which was never written with receipts at all, and
    had 107 locators with zero observations."""
