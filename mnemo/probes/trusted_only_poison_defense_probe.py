...
# 4. FAIL CLOSED: with no trust_seeds there is no trust root, so nothing can be anchored to it and the
# honest answer is "no trusted memories". This assertion was inverted until now — it still demanded the
# pre-1.19.0 fail-OPEN behaviour, which 1.19.0 deliberately reversed (a security flag that returns the
# whole untrusted pool hands back exactly the poisoned records the caller asked to exclude). The probe
# had therefore been reporting FAILED against correct code, which is worse than no test: a permanently
# red suite teaches you to ignore red.
m4 = build(trust_on=False)
check("4 trusted_only fails CLOSED when no trust_seeds (empty, not the untrusted pool)",
      len(m4.recall(Q, k=3, trusted_only=True)) == 0)
